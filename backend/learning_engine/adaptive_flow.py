...
6. **Mermaid Diagram** (concept relationship map):  
   - Create a valid Mermaid.js flowchart using `graph TD`.  
   - Include 4-8 nodes maximum. Use short labels (1-3 words per node).  
   - Show how this atomic concept connects to prerequisites, related ideas, and outcomes.  
   - Use ONLY these node shapes: `A[Rectangle]` and `B([Rounded])`. Do NOT use curly-brace shapes.  
   - For labeled edges use: `A -->|label| B` (space after the closing pipe, NO `>` after the pipe).  
   - For plain edges use: `A --> B`.  
   - Keep it simple, valid, and parseable.
...
{{
    ...
    "mermaid_diagram": "graph TD\n    A[Main Concept] -->|relates to| B([Related Idea])\n    A --> C([Outcome])"
}}
"""
