    ...
    def post(self, request):
        ...
        if not atom.explanation or force_new:
            ...
            teaching_content = {
                ...
                'mermaid_diagram': getattr(atom, '_cached_mermaid', ''),
            }
            # Generate mermaid diagram on-the-fly for cached content
            try:
                engine = AdaptiveLearningEngine()
                if engine.groq_client:
                    mermaid_prompt = f"""Create a valid Mermaid.js flowchart for the concept \"{atom.name}\" within \"{atom.concept.name}\".
Rules:
- Start with `graph TD`
- Use 4-8 nodes with short labels (1-3 words)
- ONLY use shapes: A[Rectangle] and B([Rounded]). Do NOT use curly-brace shapes.
- For labeled edges: A -->|label| B  (space after closing pipe, no > after pipe)
- For plain edges: A --> B
- Return ONLY the mermaid code, nothing else."""
                    mermaid_resp = engine.groq_client.chat.completions.create(
                        model="llama-3.3-70b-versatile",
                        messages=[{"role": "user", "content": mermaid_prompt}],
                        temperature=0.3,
                        max_tokens=300,
                    )
                    mermaid_code = mermaid_resp.choices[0].message.content.strip()
                    if '```' in mermaid_code:
                        mermaid_code = mermaid_code.split('```')[1]
                        if mermaid_code.startswith('mermaid'):
                            mermaid_code = mermaid_code[7:]
                        mermaid_code = mermaid_code.strip()
                    teaching_content['mermaid_diagram'] = mermaid_code
            except Exception as e:
                logger.warning(f"Mermaid generation for cached content failed: {e}")
...
class GenerateHintView(APIView):
    """Generate progressive hints for a question using LLM."""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        correct_index = request.data.get('correct_index', 0)
        hint_level = int(request.data.get('hint_level', 1))  # 1=nudge, 2=detailed, 3=near-answer
        ...
        level_instructions = {
            1: "Give a SUBTLE nudge — a single sentence that hints at the general direction without revealing the answer. Be vague but helpful.",
            2: "Give a CLEAR hint — explain the key concept or principle needed to solve this. Help the student think through it, but do NOT state the answer directly.",
            3: "Give a DETAILED hint — walk through the reasoning step by step. Get very close to the answer without explicitly stating it. The student should be able to figure it out from this.",
        }

        instruction = level_instructions.get(hint_level, level_instructions[1])

        prompt = f"""You are a helpful tutor. A student is stuck on a question and needs a hint.

Topic: {atom.name} (part of {atom.concept.name})
...
Rules:
- NEVER state the correct answer letter or option text directly.
- Be encouraging and supportive.
- Keep it concise (1-3 sentences max).
- Make it educational — help the student learn, not just get the right answer.
...
Return ONLY the hint text, nothing else."""
        ...
        try:
            ...
            if engine.groq_client:
                ...
                # Fallback hints
                fallbacks = {
                    1: f"Think about what {atom.name} really means at its core.",
                    2: f"Consider how {atom.name} relates to {atom.concept.name}. What's the key principle here?",
                    3: f"Focus on the relationship between the concepts. The answer connects to the core idea of {atom.name}.",
                }
                hint_text = fallbacks.get(hint_level, fallbacks[1])
        except Exception as e:
            logger.warning(f"Hint generation failed: {e}")
            hint_text = f"Think carefully about what you learned about {atom.name}. Review the key concepts."

        return Response({
            'hint': hint_text,
            'hint_level': hint_level,
            'hint_usage': progress.hint_usage if 'progress' in dir() else 0,
        })


class GenerateRecommendedInsightsView(APIView):
    """Generate personalized learning insights after atom review."""
    permission_classes = [IsAuthenticated]

    def post(self, request):
        atom_id = request.data.get('atom_id')
        accuracy = float(request.data.get('accuracy', 0))
        final_mastery = float(request.data.get('final_mastery', 0))
        error_types = request.data.get('error_types', {})
        theta_change = float(request.data.get('theta_change', 0))
        time_ratio = float(request.data.get('time_ratio', 1.0))
        ...
        # Build rule-based insights first
        insights = []
        focus_areas = []
        strengths = []
        ...
        if accuracy >= 0.9:
            strengths.append('Excellent accuracy — you demonstrate strong understanding.')
        elif accuracy >= 0.7:
            strengths.append('Good accuracy — you have a solid grasp of the material.')
        else:
            focus_areas.append('Practice more questions to improve accuracy.')

        # Error-type specific insights
        if error_types.get('conceptual', 0) > 0:
            focus_areas.append(f'Review core concepts of {atom.name} — you had {error_types["conceptual"]} conceptual errors.')
        if error_types.get('procedural', 0) > 0:
            focus_areas.append(f'Practice step-by-step problem solving — {error_types["procedural"]} procedural errors detected.')
        if error_types.get('factual', 0) > 0:
            focus_areas.append(f'Memorize key facts and definitions — {error_types["factual"]} factual errors found.')
        if error_types.get('guessing', 0) > 0:
            focus_areas.append('Avoid guessing — take time to reason through each option.')

        # Mastery-based insights
        if final_mastery >= 0.8:
            strengths.append('High mastery level — you\'re ready for advanced topics.')
        elif final_mastery >= 0.5:
            insights.append('You\'re making progress. One more review session could solidify your understanding.')
        else:
            insights.append('This topic needs more attention. Consider re-reading the teaching material.')

        # Theta-based insights
        if theta_change > 0.3:
            strengths.append('Significant ability growth detected — your learning is accelerating.')
        elif theta_change < -0.1:
            focus_areas.append('Your ability score dipped — revisiting fundamentals may help.')

        # Time-based insights
        if time_ratio > 2.0:
            insights.append('You took longer than expected — try breaking problems into smaller steps.')
        elif time_ratio < 0.5:
            insights.append('You answered very quickly — make sure you\'re reading each question carefully.')
        ...
        try:
            engine = AdaptiveLearningEngine()
            if engine.groq_client:
                prompt = f"""Based on a student's performance on "{atom.name}" (part of {atom.concept.name}):
- Accuracy: {accuracy:.0%}
- Mastery: {final_mastery:.0%}
- Theta change: {theta_change:+.2f}
- Main error types: {', '.join(k for k, v in error_types.items() if v > 0) or 'none'}

Give ONE specific, actionable study recommendation in 2 sentences max. Be encouraging but honest. Focus on what to do NEXT."""
        ...
        return Response({
            ...
            'recommendation': recommendation,
            'atom_name': atom.name,
            'concept_name': atom.concept.name,
        })
